...
from traduire_en_ch import traduire, traduire_zh_en
...
def generation_reponse(text):
    ...
    if classe_generale=="traduction" :
        ...
        nom_maladie = traduire(entite_detectee[0][0])
        print(nom_maladie)
        similar_disease = find_similar_diseases(nom_maladie,names)
        disease = similar_disease[0][0] 
        reponse = generer_reponse(classe_medicale,disease)  #mzelt fazet l similarite w generation depuis json
        print(disease,"   ff  ", classe_medicale)
        print(reponse)
    else:
        reponse = reponse_aleatoire("intentions_generales.json", classe_generale)
        reponse= traduire(reponse)
        print(reponse)
    
    return(reponse)


# generation_reponse("Do you have information about Kidney disease?")


# generation_reponse("您有关于肾病的信息吗？")

# generation_reponse("您好")

generation_reponse("你能翻译一下这句话吗 ：治疗糖尿病的最佳方法是什么？")
